...
def download_data_and_calc_momentum(start_year, end_year):
    ...
    # 💡 수정: 6개월 모멘텀 계산을 위해 시작 연도보다 7개월 일찍 데이터 수집
...
def run_custom_momentum(monthly, mom_6m, start_year):
    ...
    for i in range(len(sim_dates) - 1):
        ...
        # 현재 날짜에 모든 공격 자산의 데이터가 존재하는지 확인
        ...
        if (off_moms < 0).all():
            ...
            # 💡 수정: 미국 중기채(IEF) 6개월 수익률이 플러스(+)면 IEF 매수, 마이너스(-)면 현금 보유
...
st.title("📈 달러 통일 맞춤형 듀얼 모멘텀 백테스터 (6개월 모멘텀)")
st.markdown("""
**[전략 룰 - 6개월 모멘텀 버전]**
1. 매월 말 **코스피(EWY), S&P500(SPY), 금(GLD)**의 최근 **6개월 수익률**을 확인합니다. (모두 달러 기준)
2. 셋 중 **하나라도 0% 이상**이면, 6개월 수익률이 **가장 높은 종목에 100% 올인**합니다.
3. 셋 다 **0% 미만(마이너스)**이면, **미국 중기채(IEF)의 6개월 수익률을 확인**합니다.
